...
1. **bias_direction** must be exactly one of: Left, Center-Left, Center, Center-Right, Right, Unclear. No other values.
...
2. **bias_justification** must be 20–40 words in plain language. It must cite at least one specific, observable feature of the article (word choice, framing, source selection, omission). It must NOT be a general assertion.

3. **credibility_flags**: Return 1–5 flags. If no flags are warranted, return an empty array [].
